...
router.post('/', [ isValidUser, isEditorOrAbove ], async (req, res) => {

    //Read and update request  
    let { email, name, permission } = req.body;

    if (permission && permission.level < req.user.permission.level) {
      let response = {
        error: {
            title: "Access Denied",
  ...
  try {
    ...
    let response = {
      success: {
          ...
          desc: "The Password for the user has been updated successfully and they have received an email with new credentials."
      }
    }
    res.status(200).json(response);
